...
import { Menu, X, LogOut, LayoutDashboard, BookOpen, Award, Briefcase, Brain } from 'lucide-react'
import { useAuth } from '@/hooks/useAuth'
import { useLang } from '@/hooks/useLanguage'

export function StudentLayout() {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false)
  const { studentLogout, studentName } = useAuth()
  const { t } = useLang()
  const navigate = useNavigate()
  const location = useLocation()

  const menuItems = [
    { label: t('dashboard'), path: '/student', icon: LayoutDashboard },
    { label: t('my_courses'), path: '/student/courses', icon: BookOpen },
    { label: t('my_coc'), path: '/student/coc', icon: Award },
    { label: t('my_attachments'), path: '/student/attachments', icon: Briefcase },
    { label: t('competencies'), path: '/student/competencies', icon: Brain },
  ]
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      >
        <div className="p-6 border-b border-primary-500">
          ...
          <p className="text-primary-100 text-sm">Welcome, {studentName || 'Student'}</p>
        </div>
