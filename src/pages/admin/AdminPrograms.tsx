...
export function AdminPrograms() {
  ...
  const handleEdit = (program: Program) => {
    ...
    setFormData({
      ...
      name_am: program.name_am,
      tvet_level: program.tvet_level,
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('programs_management')}</h1>
          <p className="text-gray-600">{t('programs_desc')}</p>
        </div>
        <button
          onClick={() => {
            setEditingProgram(null)
            ...
            {programs.map((program) => (
              <tr key={program.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 py-1 text-xs rounded-full ${program.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold text-gray-900 mb-6">
              {editingProgram ? t('edit_program') : t('add_program')}
            </h2>
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={formData.tvet_level}
                    onChange={(e) => setFormData({ ...formData, tvet_level: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                  >
